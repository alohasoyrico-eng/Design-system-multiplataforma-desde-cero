...
// El banco es el guardián transversal: la lib vive en el otro workspace.
const LIBSRC = join(__dirname, '..', '..', '..', '..', 'packages', 'flow-react', 'src')
const UI = join(LIBSRC, 'ui')
...
describe('Architecture — layer imports', () => {
  ...
  it('nobody outside growth/adapters imports an analytics SDK — the adapter is the only door', () => {
    const allSrc = [
      ...walk(join(LIBSRC, 'ui'), '.tsx'),
      ...walk(join(LIBSRC, 'ui'), '.ts'),
      ...walk(join(ROOT, 'pages'), '.tsx'),
      ...walk(join(ROOT, 'layout'), '.tsx'),
      ...walk(join(ROOT, 'app'), '.tsx'),
      ...walk(join(LIBSRC, 'growth'), '.tsx'),
      ...walk(join(LIBSRC, 'growth'), '.ts'),
...
describe('Token tiers', () => {
  const TOKENS = join(LIBSRC, 'tokens')
  ...
  it('styles.css imports ref layer before sys layer', () => {
    const stylesContent = readFileSync(join(LIBSRC, 'styles.css'), 'utf-8')
