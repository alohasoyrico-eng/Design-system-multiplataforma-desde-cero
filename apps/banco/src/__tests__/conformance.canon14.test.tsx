...
import { AuthForm } from '@alohasoyrico-eng/flow-react'
import { WidgetFrame } from '@alohasoyrico-eng/flow-react'
import { SettingsRow } from '@alohasoyrico-eng/flow-react'
import { Wizard } from '@alohasoyrico-eng/flow-react'
import { GlobalSearch } from '@alohasoyrico-eng/flow-react'
...
const LIB = join(__dirname, '..', '..', '..', '..', 'packages', 'flow-react', 'src')
const fuente = (rel: string) => readFileSync(join(SRC, rel), 'utf8')
const fuenteLib = (rel: string) => readFileSync(join(LIB, rel), 'utf8')
...
describe('conformance canon · country-select-pattern', () => {
  ...
  it('pai-p5: no existe un componente SelectCountry — la receta compone Select + Flag', () => {
    expect(fuenteLib('ui/lib.ts')).not.toMatch(/SelectCountry/)
  })
})
...
describe('conformance canon · doc-hero y state-grid', () => {
  it('dh-2: los pills de plataforma son StatusPill, no HTML crudo', () => {
    expect(fuenteLib('ui/patterns/DocHero.tsx')).toMatch(/import \{ StatusPill \}/)
  })

  it('sg-1: cada celda usa Card outlined + Specimen, no HTML crudo', () => {
    const src = fuenteLib('ui/patterns/StateGrid.tsx')
